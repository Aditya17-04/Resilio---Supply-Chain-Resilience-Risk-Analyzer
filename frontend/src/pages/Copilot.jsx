...
function extractCopilotText(apiRes) {
  if (!apiRes || typeof apiRes !== 'object') return ''

  if (typeof apiRes.response_text === 'string' && apiRes.response_text.trim()) {
    return apiRes.response_text.trim()
  }
  ...
  let text = ''
  if (r.topic) text += `**${r.topic}**\n\n`
  if (r.summary) text += `${r.summary}\n\n`

  if (Array.isArray(r.top_suppliers) && r.top_suppliers.length) {
    text += '**Top Suppliers:**\n'
    r.top_suppliers.forEach((s, idx) => {
      const parts = [s?.name, s?.country ? `(${s.country})` : '', s?.riskScore != null ? `— Risk ${s.riskScore}` : '']
      text += `${idx + 1}. ${parts.filter(Boolean).join(' ')}\n`
    })
  }

  if (Array.isArray(r.key_risks) && r.key_risks.length) {
    text += `${text ? '\n' : ''}**Key Risks:**\n`
  ...
  if (r.disruption_probability != null) {
    text += `\n**Predicted Disruption Probability: ${r.disruption_probability}%**`
  }

  if (!text.trim() && typeof r.message === 'string') {
    text = r.message
  }
  ...
  const send = async (query) => {
    ...
    try {
      ...
      if (!streamedContent.trim()) {
        ...
        const response = extractCopilotText(apiRes) || 'I could not generate a response right now. Please try again.'
